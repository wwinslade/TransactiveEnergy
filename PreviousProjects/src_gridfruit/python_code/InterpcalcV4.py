...
def time2heatmed(temp):
    time2heat=291.94*temp**2 - 692.49*temp + 6705
    return time2heat
...
def temp2heatmed(time):
    temp2heat=-9e-10*time**2 + 0.0003*time + 2.3445
    return temp2heat
...
def interpolateheating(highbound, duration, loadmass):
    if loadmass>0 and loadmass<8000:#interp between low and medium mass curves, x1=0, x2=8000
        ...
        if lowtime-duration.seconds<0 or medtime-duration.seconds<0:#if event duration exceeds available precooling time
            precooltemplow=temp2heatlow(lowtime-lowtime*2/3)
            precooltempmed=temp2heatmed(medtime-medtime*2/3)
        ...
        if medtime-duration.seconds<0 or fulltime-duration.seconds<0:#if event duration exceeds available precooling time
            precooltempmed=temp2heatlow(medtime-medtime*2/3)
            precooltempfull=temp2heatmed(fulltime-fulltime*2/3)
            optprecooltemp=precooltemplow+(loadmass-0)*(precooltempfull-precooltempmed)/(8000)
        else:
