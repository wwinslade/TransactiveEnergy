...
settings={"logtrue":1,"precooltype":2,"operationmode":0,"enableADR":0,"samplerate":15,
        "Hightempboundary":3.5,"Lowtempboundary":3,"desiredtemp":3.33,"dbandoffset":0, "loadmass":16000,
...
event1start=dt.datetime(date.year,date.month,date.day,18)
event1end=dt.datetime(date.year,date.month,date.day,19)
...
def logging_loop():
    ...
    while 1: #we want this loop to always be running
        ...
        timeneededtocool1=InterpcalcV2.interpolate(settings["precoolingtemp"],tempdata[1],currentstart,settings["loadmass"])
        timeneededtocool=dt.timedelta(seconds=timeneededtocool1)
        ...
        if settings["precooltype"]==2:#interp precool
    ...
    while 1:#we want this loop to always be running
        if settings["operationmode"]==0:#set the compressor status directly from value in settings
            ...
            if settings["enableADR"]==0:#normal deadband for adr not enabled
                ...
                if varcurrent["event status"]=="during":#keep temp just slightly below high temp for duration of event
                    if varcurrent["Internal air temp"]<=settings["Hightempboundary"] and varcurrent["Internal air temp"]>=settings["Lowtempboundary"]:
                        ...
                        while varcurrent["Internal air temp"]>=settings["Hightempboundary"]-0.2:
                    ...
                    if varcurrent["Internal air temp"]<settings["precoolingtemp"]:
                        ...
                        while varcurrent["Internal air temp"]>=settings["precoolingtemp"]-0 and varcurrent['event status']=='pre':
            ...
            ## deadbands for adr enabled
                ...
                elif varcurrent["event status"]=='post':#return to OEM control
                    comppin.off()
                    varcurrent["thermpin"]=1
                    varcurrent["Compressor status"]=2
                    varcurrent["deadband status"]='N/A'
