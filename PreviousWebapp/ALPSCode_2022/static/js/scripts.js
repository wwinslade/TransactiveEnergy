...
var energyConsumptionGraph1;
var energyConsumptionGraph2;
var energyConsumptionGraph3;
var energyConsumptionGraph4;
...
var energyUsageTotals1;
var energyUsageTotals2;
var energyUsageTotals3;
var energyUsageTotals4;
...
function showUtility(){
		loadPHP("php/getEnergy1234.php", function(){
			if(xhr.readyState == 4 && xhr.status == 200){
				console.log(xhr.responseText);
//User 1				
				//document.getElementById("load-energy-consumption-script").setAttribute('type', 'text/javascript');
				document.getElementById("load-energy-consumption-script1").text = xhr.responseText;
				
				// Create the two visualizations for this page
				energyConsumptionGraph1 = new EnergyConsumptionGraph1();
				energyConsumptionGraph1.init();
			
				pieGraph1 = new PieGraph1();
				pieGraph1.init();
				...
				d3.select(window).on("resize", function(){
					pieGraph1.resize();
					energyConsumptionGraph1.resize();
				});

//				document.getElementById("load-energy-consumption-script2").text = xhr.responseText;
//User 2				
				energyConsumptionGraph2 = new EnergyConsumptionGraph2();
				energyConsumptionGraph2.init();
			
				pieGraph2 = new PieGraph2();
				pieGraph2.init();
				...
				d3.select(window).on("resize", function(){
					pieGraph2.resize();
					energyConsumptionGraph2.resize();
				});
...
//User 3	
				...
				//document.getElementById("load-energy-consumption-script").setAttribute('type', 'text/javascript');
				document.getElementById("load-energy-consumption-script3").text = xhr.responseText;
				
				energyConsumptionGraph3 = new EnergyConsumptionGraph3();
				energyConsumptionGraph3.init();
			
				pieGraph3 = new PieGraph3();
				pieGraph3.init();
				...
				d3.select(window).on("resize", function(){
					pieGraph3.resize();
					energyConsumptionGraph3.resize();
				});	

//User 4		

//				document.getElementById("load-energy-consumption-script4").text = xhr.responseText;
					
				energyConsumptionGraph4 = new EnergyConsumptionGraph4();
				energyConsumptionGraph4.init();
			
				pieGraph4 = new PieGraph4();
				pieGraph4.init();
				...
				d3.select(window).on("resize", function(){
					pieGraph4.resize();
					energyConsumptionGraph4.resize();
				});	
			}
		});

	showPage("utility", "navUtility");
	return false;
}
...
function showFrequency(){
	...
	showPage("frequency", "navFrequency");
	return false;
}
	...
	if(validateForm(formNo)){
		$.ajax({
			...
			data: {
				...
				"form-up": document.forms[formName]["form-frequency"].value,	//Peter: add the new frequency form
				"form-policy": document.forms[formName]["form-policy"].value,
				"form-number": document.forms[formName]["form-number"].value,
				"form-user": document.forms[formName]["form-user"].value
			},	
...
//Peter: Change parseInt(policy) form 2 to 3

	var policy = document.forms[formName]["form-policy"].value;
	if(isNaN(parseInt(policy)) ||  parseInt(policy) < 0 || parseInt(policy) > 3){	
		return false;
	}
...
function addAppliance(){
	$.ajax({
		...
		data: {
			...
			"form-power": 60,
			"form-threshold": 0,
			"form-frequency": 59.5, //Peter: add a new frequency form
			"form-user": 1,
			"form-policy": 0
...
//Peter added the following 3 functions
function addAppliance2(){
	$.ajax({
		type: "POST",
		url: "php/addAppliance.php",
		data: {
...
function showThreshold(applianceNum, show){
	if(show===1){
		...
		document.getElementById("frequency-div-" + applianceNum).className = "frequency-div hidden";
	}
	if(show===2){
		document.getElementById("threshold-div-" + applianceNum).className = "threshold-div hidden";
		document.getElementById("frequency-div-" + applianceNum).className = "frequency-div shown";
	}
	if(show===0){
		document.getElementById("threshold-div-" + applianceNum).className = "threshold-div hidden";
		document.getElementById("frequency-div-" + applianceNum).className = "frequency-div hidden";
